...
export class PostService {
  ...
  private _ratePostUrl = `${environment.baseApiEndPoint}/post/rate`;
  ...
  ratePost(postId: number, score: number): Observable<number> {
    const rate = { PostId: postId, Score: score };
    return this._http.post(this._ratePostUrl, rate)
        .map(r => <number>r.json())
        .do(data => console.log(`Post/Rate/:` + JSON.stringify(data)))
        .catch(this.handleError);
}
